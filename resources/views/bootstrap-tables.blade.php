<title>BTI SOFT</title>
<div class="py-4">
    <nav aria-label="breadcrumb" class="d-none d-md-inline-block">
        <ol class="breadcrumb breadcrumb-dark breadcrumb-transparent">
            <li class="breadcrumb-item">
                <a href="#">
                    <svg class="icon icon-xxs" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"></path></svg>
                </a>
            </li>
            <li class="breadcrumb-item"><a href="#">Docentes</a></li>
            <li class="breadcrumb-item active" aria-current="page">Retículas-Planes de estudio</li>
        </ol>
    </nav>
    <div class="d-flex justify-content-between w-100 flex-wrap">
        <div class="mb-3 mb-lg-0">
            <h1 class="h4">Retículas-Planes de estudio</h1>
            <p class="mb-0">Docente, aquí podrás encontrar de manera general los planes de estudio de cada una de las materias impartidas.</p>
        </div>
    </div>
</div>

<div class="card border-0 shadow mb-4">
    <div class="card-body">

        <!-- TABS -->
        <ul class="nav nav-tabs" id="tabsMaterias" role="tablist">
            <li class="nav-item">
                <button class="nav-link active" data-bs-toggle="tab" data-bs-target="#tab1">1ER SEMESTRE</button>
            </li>
            <li class="nav-item">
                <button class="nav-link" data-bs-toggle="tab" data-bs-target="#tab2">2DO SEMESTRE</button>
            </li>
            <li class="nav-item">
                <button class="nav-link" data-bs-toggle="tab" data-bs-target="#tab3">3ER SEMESTRE</button>
            </li>
            <li class="nav-item">
                <button class="nav-link" data-bs-toggle="tab" data-bs-target="#tab3">4TO SEMESTRE</button>
            </li>
            <li class="nav-item">
                <button class="nav-link" data-bs-toggle="tab" data-bs-target="#tab3">5TO SEMESTRE</button>
            </li>
            <li class="nav-item">
                <button class="nav-link" data-bs-toggle="tab" data-bs-target="#tab3">6TO SEMESTRE</button>
            </li>
        </ul>

        <!-- CONTENIDO -->
        <div class="tab-content mt-3">

            <!-- ================= SEMESTRE 1 ================= -->
            <div class="tab-pane fade show active" id="tab1">
                <div class="row g-3">

                    <div class="col-md-6"><div class="card"><div class="card-body"><h5>Matemáticas I</h5><a href="#" class="btn btn-info">Descargar</a></div></div></div>
                    <div class="col-md-6"><div class="card"><div class="card-body"><h5>Química I</h5><a href="#" class="btn btn-info">Descargar</a></div></div></div>
                    <div class="col-md-6"><div class="card"><div class="card-body"><h5>Geografía</h5><a href="#" class="btn btn-info">Descargar</a></div></div></div>
                    <div class="col-md-6"><div class="card"><div class="card-body"><h5>Introducción a Ciencias Sociales I</h5><a href="#" class="btn btn-info">Descargar</a></div></div></div>
                    <div class="col-md-6"><div class="card"><div class="card-body"><h5>Informática I</h5><a href="#" class="btn btn-info">Descargar</a></div></div></div>
                    <div class="col-md-6"><div class="card"><div class="card-body"><h5>Lectura y Redacción I</h5><a href="#" class="btn btn-info">Descargar</a></div></div></div>
                    <div class="col-md-6"><div class="card"><div class="card-body"><h5>Inglés I</h5><a href="#" class="btn btn-info">Descargar</a></div></div></div>

                </div>
            </div>

            <!-- ================= SEMESTRE 2 ================= -->
            <div class="tab-pane fade" id="tab2">
                <div class="row g-3">

                    <div class="col-md-6"><div class="card"><div class="card-body"><h5>Matemáticas II</h5><a href="#" class="btn btn-info">Descargar</a></div></div></div>
                    <div class="col-md-6"><div class="card"><div class="card-body"><h5>Química II</h5><a href="#" class="btn btn-info">Descargar</a></div></div></div>
                    <div class="col-md-6"><div class="card"><div class="card-body"><h5>Biología I</h5><a href="#" class="btn btn-info">Descargar</a></div></div></div>
                    <div class="col-md-6"><div class="card"><div class="card-body"><h5>Historia de México I</h5><a href="#" class="btn btn-info">Descargar</a></div></div></div>
                    <div class="col-md-6"><div class="card"><div class="card-body"><h5>Informática II</h5><a href="#" class="btn btn-info">Descargar</a></div></div></div>
                    <div class="col-md-6"><div class="card"><div class="card-body"><h5>Lectura y Redacción II</h5><a href="#" class="btn btn-info">Descargar</a></div></div></div>
                    <div class="col-md-6"><div class="card"><div class="card-body"><h5>Inglés II</h5><a href="#" class="btn btn-info">Descargar</a></div></div></div>

                </div>
            </div>

            <!-- ================= SEMESTRE 3 ================= -->
            <div class="tab-pane fade" id="tab3">
                <div class="row g-3">

                    <div class="col-md-6"><div class="card"><div class="card-body"><h5>Matemáticas III</h5><a href="#" class="btn btn-info">Descargar</a></div></div></div>
                    <div class="col-md-6"><div class="card"><div class="card-body"><h5>Física I</h5><a href="#" class="btn btn-info">Descargar</a></div></div></div>
                    <div class="col-md-6"><div class="card"><div class="card-body"><h5>Metodología de la Investigación I</h5><a href="#" class="btn btn-info">Descargar</a></div></div></div>
                    <div class="col-md-6"><div class="card"><div class="card-body"><h5>Historia de México II</h5><a href="#" class="btn btn-info">Descargar</a></div></div></div>
                    <div class="col-md-6"><div class="card"><div class="card-body"><h5>Biología II</h5><a href="#" class="btn btn-info">Descargar</a></div></div></div>
                    <div class="col-md-6"><div class="card"><div class="card-body"><h5>Literatura I</h5><a href="#" class="btn btn-info">Descargar</a></div></div></div>
                    <div class="col-md-6"><div class="card"><div class="card-body"><h5>Inglés III</h5><a href="#" class="btn btn-info">Descargar</a></div></div></div>

                </div>
            </div>

        </div>

    </div>
</div>
</div>
